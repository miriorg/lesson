...
function sheetRename(wb: ExcelScript.Workbook, strSheet: string): ExcelScript.Worksheet {
  ...
  if (sheet == void 0) { //生成シートが存在しない場合はundefinedが返ってくるので、空シートを追加
    ...
    //backupのシートを探す
    let sheetBackup = wb.getWorksheet(`${strSheet}${dtNow}`); //backupシートを取得
    if (sheetBackup == void 0) { //backupシートが存在しない場合はundefinedが返ってくるので、既存シートをrenameして空シートを追加
      sheet.setName(`${strSheet}${dtNow}`); //既存シートをrenameしてバックアップ
      sheet = wb.addWorksheet(strSheet);  //空シートを生成
      arLatehiding.push(`${strSheet}${dtNow}`); //隠すシート名をメモ(転記できなくなるので先に隠しちゃダメ)
    }
    else { //バックアップシートが存在する場合は特に処理無し
    }
  }
  sheet.setPosition(0);
  return sheet;
...
function detectSource(sheets: ExcelScript.Worksheet[]): ExcelScript.Worksheet {
  const yymmdd = /20[234][0-9][01][0-9][0123][0-9]/
  const regex = /[Kk][Aa][Ee][Tt][Ee][Cc]20[234][0-9][01][0-9][0123][0-9]/
